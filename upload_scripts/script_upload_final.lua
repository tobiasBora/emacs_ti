#!/usr/bin/lua
-- First part from :
-- https://github.com/jimbauwens/ETK/blob/master/buildtools/build.lua
-- Ce script regroupe la partie conversion et la partie d'envoie vers la
-- calculatrice.

commande_envoi = "ti-upload-file"
commande_luna = "luna"
print("+--------------------------------------------+")
print("|   Send and convert lua file to TI calc     |")
print("+--------------------------------------------+")
print("   ==> Tobias BORA, first part from jimbauwens")      
...
-- Cette fonction cherche dans le fichier si elle trouve
-- la chaine :
--Ti-mode="<folder>"
...
function send(filename)
   ...
   os.execute(commande_envoi .. " " .. filename .. " " .. folder)
...
assert(os.execute("cat " .. tmpname .. "|" .. commande_luna .. " - " .. outfilename), "Building failed! You can try to build " .. tmpname .. " manually")
print("\r\nBuilding successful!")
